...
async def test_explain_workflow():
    ...
    print(
        f'✅ Properties being explained present: {"properties_being_explained" in explain_result}'
    )
    ...
    if execution_token and execution_token in _properties_store:
        ...
        print(
            f'✅ Stored properties count: {len(stored_props) if isinstance(stored_props, dict) else "Not a dict"}'
        )
    else:
