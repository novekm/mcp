...
def validate_security_check_result(security_check_result, resource_type, skip_security_check=False):
    ...
    if 'timestamp' in security_check_result:
        try:
            check_time = datetime.datetime.fromisoformat(security_check_result['timestamp'].replace('Z', '+00:00'))
    ...
    if security_check_result.get('resource_type') and security_check_result.get('resource_type') != resource_type:
        raise ClientError(
        ...
        if failed_checks:
            ...
            if high_severity_issues:
                ...
                for issue in high_severity_issues:
                    ...
                    security_findings += f"  Description: {issue.get('description', 'No description')}\n"
