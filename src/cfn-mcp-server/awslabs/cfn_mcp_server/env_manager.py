...
def load_environment_variables() -> Dict[str, str]:
    ...
    try:
        session_duration = int(env_vars['SESSION_DURATION'])
        if not (1 <= session_duration <= 24):
            print(f"Warning: SESSION_DURATION must be between 1 and 24. Using default value of 8.")
            env_vars['SESSION_DURATION'] = '8'
    except ValueError:
        print(f"Warning: SESSION_DURATION must be an integer. Using default value of 8.")
        env_vars['SESSION_DURATION'] = '8'
