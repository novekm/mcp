...
def add_default_tags(
    resource_properties: Dict[str, Any], schema: Dict[str, Any] = None
) -> Dict[str, Any]:
    ...
    default_tags_enabled = os.environ.get(
    ...
    if 'Tags' in updated_properties:
        if isinstance(updated_properties['Tags'], list):
            ...
            existing_tags = [{'Key': k, 'Value': v}
                             for k, v in updated_properties['Tags'].items()]
    ...
    filtered_existing_tags = [
        tag for tag in existing_tags if tag['Key'] not in default_tag_keys]
