...
session_config = botocore.config.Config(
    user_agent_extra='ccapi-mcp-server/1.0.0',
)
...
def get_aws_client(service_name, region_name=None):
    ...
    try:
        if cred_source == 'env' or cred_source == 'environment':
            ...
            print(
                f'Creating {service_name} client using environment variables')
            ...
            print(
                f'Creating {service_name} client using profile: {profile_name}')
            ...
            print(
                f'Creating {service_name} client using SSO profile: {profile_name}')
            session = Session(profile_name=profile_name)
        elif cred_source == 'instance' or cred_source == 'role':
            # Force use of instance profile/container role
            print(
                f'Creating {service_name} client using instance profile/container role')
            session = Session(aws_access_key_id=None,
                              aws_secret_access_key=None)
        else:  # 'auto' or any other value
            # Use default credential provider chain
            print(
                f'Creating {service_name} client using default AWS credential provider chain')
            session = Session()

        client = session.client(
            service_name, region_name=region_name, config=session_config)
        ...
        print(
            f'Successfully created {service_name} client for region {region_name}')
        return client

    except Exception as e:
        print(
            f'Error creating {service_name} client: {str(e)}', file=sys.stderr)
