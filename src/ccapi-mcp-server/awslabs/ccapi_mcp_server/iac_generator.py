...
async def create_template(
    ...
    if not template_id and not template_name:
        raise ClientError(
            'Either template_name or template_id must be provided')
    ...
    if deletion_policy not in ['RETAIN', 'DELETE', 'SNAPSHOT']:
        raise ClientError(
            "deletion_policy must be one of 'RETAIN', 'DELETE', or 'SNAPSHOT'")

    if update_replace_policy not in ['RETAIN', 'DELETE', 'SNAPSHOT']:
        raise ClientError(
            "update_replace_policy must be one of 'RETAIN', 'DELETE', or 'SNAPSHOT'")
        ...
        if save_to_file:
            try:
                ...
                os.makedirs(os.path.dirname(
                    os.path.abspath(save_to_file)), exist_ok=True)
