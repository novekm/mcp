...
class SchemaManager:
    ...
    async def _download_resource_schema(
        ...
        try:
            print(
                f'Downloading schema for {resource_type} using CloudFormation API')
            cfn_client = get_aws_client('cloudformation', region)
            resp = cfn_client.describe_type(
                Type='RESOURCE', TypeName=resource_type)
            schema_str = resp['Schema']
            spec = json.loads(schema_str)

            # Save schema to cache
            schema_file = self.cache_dir / \
                f'{resource_type.replace("::", "_")}.json'
            ...
            raise ClientError(
                f'Error downloading the schema for {resource_type}: {str(e)}')
